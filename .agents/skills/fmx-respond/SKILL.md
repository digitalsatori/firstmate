...
This skill is the sole owner of the completion-follow-up procedure below; this skill's own `description:` line declares the load trigger for Relay-linked milestone or terminal wakes, with the `agent-only-skills` trigger index as the pointer that disambiguates, and AGENTS.md §8 reinforces the terminal final-follow-up step before teardown.
...
## Relay activation and posture (extracted from AGENTS.md section 14)

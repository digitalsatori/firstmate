...
state/               runtime records and signals; gitignored
  ...
  when/              private condition->action watch specs, their trust bindings, and single-fire markers; written only by bin/fm-procevent-when.sh (the `process-event-sources` trigger in `agent-only-skills`)

...
`docs/configuration.md` owns the operator-facing summary of the top-level operational-home layout and the configuration schemas; each producing script's header and help own exact child fields and mutation mechanics.
...
The full operational-home layout - every top-level entry and every `state/` record, with its owner and touch rules - lives in `.agents/skills/home-layout/SKILL.md`; load it before creating, reading, moving, or deleting any file under `data/`, `state/`, `config/`, `projects/`, or `.no-mistakes/`, or before interpreting any task status, metadata, check, poll, inbox, lease, or afk record.
...
`docs/configuration.md` owns dispatch-profile and runtime-backend schemas, `bin/fm-harness.sh` owns static resolution, and `bin/fm-spawn.sh` owns launch flags and fail-closed validation; when dispatch profiles exist, load `.agents/skills/dispatch-profiles/SKILL.md` at every crewmate or scout intake, and load `quota-array-dispatch` before ranking a matched profile array.
...
`bin/fm-brief.sh` and its help own scaffold syntax, generated variants, status protocol, definitions of done, and exact safety mechanics; load `.agents/skills/crewmate-briefs/SKILL.md` before writing, scaffolding, or editing any ship or scout brief.

## 12. Self-update

When the captain invokes `/updatefirstmate` or asks to update firstmate, load the `/updatefirstmate` skill, which owns the guarded fleet update and restart procedure.
...
Relay is the public-mention integration (inert until `FMX_PAIRING_TOKEN` is set in `.env`); load the `fmx-respond` skill before promising, handling, or delivering any public reply, and see `docs/configuration.md` for activation and opt-out mechanics.

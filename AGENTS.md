...
The operational-home layout reference lives in `.agents/skills/home-layout/SKILL.md`; load it before creating, reading, moving, or deleting any file under `data/`, `state/`, `config/`, `projects/`, or `.no-mistakes/`, or before interpreting any task status, metadata, check, poll, inbox, lease, or afk record.

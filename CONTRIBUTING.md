...
- This repo is a template for running a firstmate orchestrator agent.
  [`AGENTS.md`](AGENTS.md) owns the supervisor contract and role boundary, while each bundled firstmate skill's own `description:` line is its load trigger and `.agents/skills/agent-only-skills/SKILL.md` carries the consolidated trigger table; `CLAUDE.md` is a real `@AGENTS.md` pointer to it, and `.claude/skills` is a symlink to `.agents/skills`.

...
- This repo is a template for running a firstmate orchestrator agent.
  [`AGENTS.md`](AGENTS.md) owns the supervisor contract and role boundary, while an agent-only skill declares its load condition in its own `description:` line and `.agents/skills/agent-only-skills/SKILL.md` indexes the reference skills it lists; `CLAUDE.md` is a real `@AGENTS.md` pointer to it, and `.claude/skills` is a symlink to `.agents/skills`.
